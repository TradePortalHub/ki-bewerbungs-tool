...
async function bootstrap() {
  try {
    ...
    const jobs = Array.isArray(cfg.jobs?.postings) ? cfg.jobs.postings : [];
...
function handleUrlChanges() {
  let lastCompany = window.KIBT_SETTINGS?.company;
  let lastJobId = new URL(window.location.href).searchParams.get('job_id');

  setInterval(() => {
    const currentCompany = window.KIBT_SETTINGS?.company;
    const currentJobId = new URL(window.location.href).searchParams.get('job_id');
    
    if (currentCompany !== lastCompany || currentJobId !== lastJobId) {
